...
# Read Training data --------------------------------------------------
print('> Loading data... ')
df1=pd.read_csv('train_input_RSS.csv')
df1= np.asarray(df1)
df1=(df1 + 100) / 100
df2=pd.read_csv('train_input_location.csv')
df2= np.asarray(df2)
...
####################################################################
# RSSI array
TestData_origin =pd.read_csv('test_input_RSS.csv')
...
# List of Locations 
Location_origin = pd.read_csv('test_input_location.csv')
...
for CountFold in range(1, N_fold):
    # training samples -----------------------
    ...
    for ep in range(epochs_num):
      ...
      #  File3.write(str(hist.history['loss'][0]) + '\n') #100 write to file
        ...
        for iSample in range(0,len(df1_split)):
    ...
    Acc_Location[0,:] = L1
    L_combine = np.concatenate((L1,L2), axis=0)
    RSSI_combine = np.concatenate((RSSI_L2, RSSI_L1), axis=0)
    ...
    for Step in range(1,timestep):
        ...
        for i in range(timestep):
            ...
            for j in range(len(RSSI_L2)):
                TestingData[j+LocationIdx] = (float(RSSI_combine[j+RSSIdx])+100)/100
        ...
        IdxTemp = 0
        # Update Location
        for j in range(Step*2,len(L_combine)):
        ...
        # Take RSSI
        IdxTemp = 0
        for j in range(Step*num_rssi_reading,len(RSSI_combine)):
    ...
    for CountTest in range(LengthTest-timestep):
    ...
    # File2.write(str(Final_L[1]) + '\n') # write to file
        ...
        if CountTest+timestep+1 < LengthTest:
            for t in range(timestep-1):
                for k in range(num_rssi_reading):
